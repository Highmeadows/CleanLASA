...
function, not for
[`haven::read_sav()`](https://haven.tidyverse.org/reference/read_spss.html).
They are validated against that function's formal arguments and then
passed through with [`do.call()`](https://rdrr.io/r/base/do.call.html).
For example, when file 046 is detected, `to_factor = TRUE`,
`to_numeric = TRUE`, `standardize_names = TRUE`, and
`name_corrections =` `...` are passed directly to
[`apply_lasa046_labels()`](https://highmeadows.github.io/CleanLASA/reference/apply_lasa046_labels.md).
